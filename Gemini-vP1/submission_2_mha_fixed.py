...
def apply_rope(x, seq_len):
    ...
    # dtype fix: cast to input dtype so bf16 training doesn't throw RuntimeError
    cos_f = freqs.cos().to(x.dtype)
    sin_f = freqs.sin().to(x.dtype)
    x1, x2 = x[..., ::2], x[..., 1::2]
    return torch.stack([x1 * cos_f - x2 * sin_f, x1 * sin_f + x2 * cos_f], dim=-1).flatten(-2)


class Pre_Norm___MHA___Residual(nn.Module):
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.Attn_Norm = RMSNorm(1024)
        self.W_Q = nn.Linear(1024, 1024, bias=False)
        self.W_K = nn.Linear(1024, 1024, bias=False)
    ...
    def forward(self, x, residual=None):
        ...
        k = self.W_K(h).view(bsz, seq_len, 16, 64).transpose(1, 2)
        v = self.W_V(h).view(bsz, seq_len, 16, 64).transpose(1, 2)
...
class Pre_Norm___SwiGLU___Residual(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.W_gate = nn.Linear(1024, 4704, bias=False)
        self.W_up = nn.Linear(1024, 4704, bias=False)
        self.W_down = nn.Linear(4704, 1024, bias=False)
...
class TransformerModel(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.layers = nn.ModuleList([
            nn.ModuleList([
                Pre_Norm___MHA___Residual(config),
